...
void AATagPlayer::SetupPlayerInputComponent(
    ...
    PlayerInputComponent->BindAxis("MoveForward", this,
                                   &AATagPlayer::MoveForward);
    PlayerInputComponent->BindAxis("MoveRight", this,
                                   &AATagPlayer::MoveRight);
    ...
    PlayerInputComponent->BindAxis("TurnRate", this,
                                   &AATagPlayer::TurnAtRate);
    PlayerInputComponent->BindAxis("LookUp", this,
                                   &APawn::AddControllerPitchInput);
    PlayerInputComponent->BindAxis("LookUpRate", this,
                                   &AATagPlayer::LookUpAtRate);
}

void AATagPlayer::TouchStarted(
    ETouchIndex::Type FingerIndex, FVector Location
) {
    Jump();
}

void AATagPlayer::TouchStopped(
    ETouchIndex::Type FingerIndex, FVector Location
) {
    StopJumping();
}

void AATagPlayer::TurnAtRate(float Rate) {
    // calculate delta for this frame from the rate information
    AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void AATagPlayer::LookUpAtRate(float Rate) {
    // calculate delta for this frame from the rate information
    AddControllerPitchInput(
        Rate * BaseLookUpRate * GetWorld()->GetDeltaSeconds());
}

void AATagPlayer::MoveForward(float Value) {
...
void AATagPlayer::MoveRight(float Value) {
    if ((Controller != NULL) && (Value != 0.0f)) {
